...
train_dataloader = torch.utils.data.DataLoader(datasets.MNIST('../data/mnist',train=True,
...
class trainer_gan(nn.Module):
    def __init__(self):
        ...
        # self.Optimizer_G = torch.optim.Adam(self.G.parameters(), lr=opt.lr, betas=(0.05, 0.5))
        # self.Optimizer_D = torch.optim.Adam(self.D.parameters(), lr=opt.lr, betas=(0.05, 0.5))
        # dcgan_1
    ...
    def test(self):
        ...
        self.G.load_state_dict(torch.load('%s/generator_080.pkl' %(opt.save_path)))
        print('1')
